...
- `targets/hosts/ms-s1-max/config/capabilities.nix` dit explicitement que la machine porte `ollama`, `llama-cpp`, `hf`, `PostgreSQL`, `pgweb`, `Steam`, `Lutris`, `Proton`, `Wine`, `gamescope`, `gamemode`, `Podman Desktop`, `btop`, `opencode-desktop`, `Rider`, `WebStorm`, `VS Code`, `GitKraken` et `Flatpak`
